...
def select_context(states,message,limit,registry=None):
    ...
    for state in states:
        ...
        if any(h in searchable for h in hints):
            candidate={"entity_id":entity_id,"state":redact(state.get("state")),"current_temperature":redact(attrs.get("current_temperature")),"unit":redact(attrs.get("unit_of_measurement") or attrs.get("temperature_unit")),"name":redact(attrs.get("friendly_name")),"device_class":redact(attrs.get("device_class")),"area":redact(meta.get("area")),"floor":redact(meta.get("floor")),"device":redact(meta.get("device")),"last_updated":state.get("last_updated")}
...
def verified_floor_mappings(states,registry):
    ...
    for state in states:
        ...
        attrs=state.get("attributes",{})
        sensor_temperature=entity_id.startswith("sensor.") and attrs.get("device_class")=="temperature"
        climate_temperature=entity_id.startswith("climate.") and attrs.get("current_temperature") is not None
        if not (sensor_temperature or climate_temperature): continue
        value=attrs.get("current_temperature") if climate_temperature else state.get("state")
        try: float(value)
        ...
        score=(100 if floor_bucket(meta.get("floor"))==bucket else 0)+(40 if floor_bucket(meta.get("area"))==bucket else 0)+(10 if meta.get("device") else 0)+(5 if sensor_temperature else 0)
